...
// each time 
// now is associated with millis and because of the chronometer that has been started just at the R4 boot moment l-62, and the values took by the "prochain coup" i millis with a max value "4000ms", we can exactly know when we got the "choosed value" if we got the millis chrono superior to this choosed value... 
...
// we associate the 0 value of "temps phase" with now to restart the now chrono...
