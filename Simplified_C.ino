...
unsigned long tempsPhase = 0; // "unsigned long" mean a range of numbers from 0 to 4,294,967,295, negatives int aren't there for this function...

// so "tempsPhase" is equal to 0 for l.23...
...
// for this l-27, we need to initialise the chrono values to 0 before "prochainCoup" take a chrono value in line 62...
...
void setup() {//*
  ...
  prochainCoup = millis() + random(1000, 4000); // we start a chronometer with millis and start the random number generation...
 //"millis" launcher appears only one time to initialise the "prochainCoup" that will be executed only one time, and then we need a function to reload thoses futur millis values after the initialisation to get our "millis" values right again and again...
}//*
...
// now is associated with millis and because of the chronometer that has been started just at the R4 boot moment l-62, and the random values took by the "prochain coup" in millis with a max value "4000ms", we can exactly know when we got the "choosed value" if we got the millis chrono superior to this choosed value... 
       ...
       Etat etatActuel = ATTAQUE; // in logic, just after the green led got emmited from the "REPOS" section/part, we are now in the "ATTAQUE" part of the game, like a linear logic evolution of the game events... 
...
// temps phase = 0 so after we got 350ms after the mllis restart line-93...
        ...
        if (distance <= 100) { // we also add a precision in the IA, so we need to allow the US sensor recognisinng the inferior distance from 100cm...
         ...
         Etat etatActuel = TOUCHE; // logic linear state that comes after "ATTAQUE"...
         //2nd cond...
...
// this funnction is necessary beacause we need to reload the millis value each time we are restaritng the REPOS case, after the first millis initialisation line 62 has been loaded...

  prochainCoup = millis() + random(1500, 4000); 
// "millis" shows us when the moment in seconds since the R4 has been boot and "random" chooses for us a value between 1000 and 4000 ms, if we summ both of thises "millis" and "random" values, we have a delay that dont freeze our AT-mega processor each time we need to react; 
 
Etat etatActuel = REPOS;  // we associate this function to the game main sleep mode... 
